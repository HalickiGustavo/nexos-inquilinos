...
export function TenantShell() {
    ...
    return (<div className="min-h-screen flex flex-col bg-muted/30">
      <MobileDrawer brand={{ to: "/tenant" }} navGroups={[{ items: tenantNav.map(item => ({ ...item })) }]} alerts={{ alerts, seeAllHref: "/tenant/alertas" }}/>
      ...
      <header className="hidden md:block fixed top-0 inset-x-0 z-40 backdrop-blur-xl bg-card/85 border-b border-border/60">
        <div className="max-w-3xl mx-auto px-4 h-14 flex items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-1 sm:gap-2 shrink-0">
            ...
            <Button variant="ghost" size="icon" aria-label="Sair" onClick={async () => {
            await signOut();
      ...
      <nav aria-label="Navegação principal" className="fixed bottom-0 inset-x-0 z-30 backdrop-blur-xl bg-card/90 border-t border-border/60 md:hidden pb-[env(safe-area-inset-bottom)] print:hidden">
        <div className="flex overflow-x-auto overscroll-x-contain [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
          {tenantNav.map((item) => {
            const active = isActive(item.to, item.exact);
            const Icon = item.icon;
            return (<Link key={item.to} to={item.to} data-tour={item.tour} aria-current={active ? "page" : undefined} className={cn("relative flex flex-1 shrink-0 basis-[68px] flex-col items-center justify-center gap-1 py-2 text-[11px] transition-colors", active ? "text-primary" : "text-muted-foreground hover:text-foreground active:bg-muted/50")}>
                {active && (<span className="absolute top-0 left-1/2 -translate-x-1/2 h-0.5 w-6 rounded-full bg-primary" aria-hidden/>)}
                <span className="relative">
                  <Icon className={cn("size-[18px] shrink-0", active && "text-primary")}/>
                  {item.to === "/tenant/chat" && unread > 0 && (<span className="absolute -top-1.5 -right-2 min-w-4 h-4 px-1 rounded-full bg-destructive text-destructive-foreground text-[9px] font-bold flex items-center justify-center">
                      {unread > 9 ? "9+" : unread}
                    </span>)}
                </span>
      ...
      <nav className="hidden md:flex border-b border-border/60 backdrop-blur-xl bg-card/85 fixed top-14 left-0 right-0 z-30">
        <div className="max-w-3xl mx-auto px-4 flex gap-1 w-full overflow-x-auto">
          {tenantNav.map((item) => {
            const active = isActive(item.to, item.exact);
            const Icon = item.icon;
            return (<Link key={item.to} to={item.to} data-tour={item.tour} className={cn("flex items-center gap-2 px-4 py-3 text-sm border-b-2 transition-colors whitespace-nowrap", active
                    ? "border-primary text-primary font-semibold"
                    : "border-transparent text-muted-foreground hover:text-foreground hover:border-border")}>
                <Icon className="size-4"/>
                {item.label}
                {item.to === "/tenant/chat" && unread > 0 && (<span className="min-w-[18px] h-[18px] px-1 rounded-full bg-destructive text-destructive-foreground text-[10px] font-bold flex items-center justify-center">
                    {unread > 9 ? "9+" : unread}
                  </span>)}
              </Link>);
        })}
