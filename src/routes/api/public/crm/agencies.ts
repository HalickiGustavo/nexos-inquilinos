        ...
        try {
          ...
          const formattedAgencies = await Promise.all((agencies || []).map(async (a: any) => {
            const [propCount, tenantCount, contractCount] = await Promise.all([
              supabaseAdmin.from('properties').select('*', { count: 'exact', head: true }).eq('user_id', a.manager_user_id as any),
              supabaseAdmin.from('tenants').select('*', { count: 'exact', head: true }).eq('user_id', a.manager_user_id as any),
              supabaseAdmin.from('contracts').select('*', { count: 'exact', head: true }).eq('user_id', a.manager_user_id as any)
            ]);
