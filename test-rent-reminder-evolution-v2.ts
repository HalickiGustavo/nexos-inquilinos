...
async function test() {
  // Forçamos a Evolution API desativando temporariamente as chaves do SendPulse
  delete process.env.SENDPULSE_CLIENT_ID;
  delete process.env.SENDPULSE_CLIENT_SECRET;
  
  // Usamos o nome da instância correta que encontramos (Nexo suporte)
  process.env.EVOLUTION_API_INSTANCE = "Nexo suporte";

  const phone = "5541987771358";
  const text = "Olá! Este é um lembrete automático de teste da Nexo (via Evolution API - Nexo suporte).\n\nIdentificamos o aluguel deste mês no valor de R$ 1.250,50 com vencimento em 15/08/2026 pendente.\n\nPor favor, ignore esta mensagem, é apenas um teste de integração.";

  console.log("Iniciando envio de mensagem de teste via Evolution API...");
  console.log("Instância:", process.env.EVOLUTION_API_INSTANCE);
