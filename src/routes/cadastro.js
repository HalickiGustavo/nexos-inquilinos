...
const ReCAPTCHA = ReCAPTCHAModule.default?.default ??
    ReCAPTCHAModule.default ??
    ReCAPTCHAModule;
import { toast } from "sonner";
import { Building2, Home as HomeIcon, Loader2, Mail, Lock, Eye, EyeOff, CheckCircle2, ArrowRight, ArrowLeft, Smartphone, Download, User, Phone, IdCard, Calendar, AlertCircle, } from "lucide-react";
...
import { isValidCNPJ, isValidCPF, isValidPhone, maskCNPJ, maskCPF, maskPhone, onlyDigits, scorePassword, } from "@/lib/br-validators";
...
const ALLOWED_ROLES = ["imobiliaria", "proprietario"];
export const Route = createFileRoute("/cadastro")({
    ...
    validateSearch: (search) => {
        ...
        return {
            role: raw && ALLOWED_ROLES.includes(raw) ? raw : undefined,
            invite,
        };
    },
...
function CadastroPage() {
    ...
    const [role, setRole] = useState(roleParam ?? (invite ? "proprietario" : null));
    useEffect(() => {
    ...
    return (<div className="min-h-screen bg-zinc-50 text-zinc-900">
      <div className="flex min-h-[100dvh] flex-col items-center justify-center p-4 sm:p-6">
        <div className="w-full max-w-md">
          ...
          {role === null ? (<RoleSelector onPick={(r) => navigate({ to: "/cadastro", search: { role: r } })}/>) : (<OnboardingWizard role={role} onChangeRole={() => navigate({ to: "/cadastro", search: {} })}/>)}
        </div>
      </div>
    </div>);
}
...
function RoleSelector({ onPick }) {
    return (<div className="rounded-2xl border border-zinc-200 bg-white shadow-sm p-6 sm:p-8">
      ...
      <div className="space-y-2.5">
        <RoleCard icon={<Building2 className="size-5"/>} title="Imobiliária" description="Gestão de carteira, equipe e repasses." onClick={() => onPick("imobiliaria")}/>
        <RoleCard icon={<HomeIcon className="size-5"/>} title="Proprietário" description="Controle direto dos seus imóveis." onClick={() => onPick("proprietario")}/>
      </div>
    </div>);
}
function RoleCard({ icon, title, description, onClick, }) {
    return (<button type="button" onClick={onClick} className="group w-full text-left rounded-xl border border-zinc-200 bg-white p-4 flex items-center gap-4 transition-all hover:border-violet-500/70 hover:bg-zinc-50 focus:outline-none focus-visible:ring-2 focus-visible:ring-violet-500">
...
function OnboardingWizard({ role, onChangeRole }) {
    const navigate = useNavigate();
    ...
    return (<div className="rounded-2xl border border-zinc-200 bg-white shadow-sm p-6 sm:p-8">
      <div className="flex items-start justify-between gap-3 mb-6">
        ...
        <button onClick={onChangeRole} className="text-xs text-zinc-500 hover:text-violet-600 transition-colors shrink-0" type="button">
          Trocar perfil
        </button>
      </div>
      ...
      <div className="mt-7">
        {step === 1 && (<StepCredentials form={form} update={update} captchaRef={captchaRef} onNext={() => setStep(2)}/>)}
        {step === 2 && (<StepIdentity role={role} form={form} update={update} onBack={() => setStep(1)} onNext={() => setStep(3)}/>)}
        {step === 3 && (<StepTerms form={form} update={update} submitting={submitting} onBack={() => setStep(2)} onSubmit={handleSubmit}/>)}
      </div>
    </div>);
}
function StepperBar({ step }) {
    ...
    return (<div className="flex items-center">
      {items.map((it, idx) => {
            ...
            return (<div key={it.n} className="flex items-center flex-1 last:flex-none">
            <div className="flex items-center gap-2 min-w-0">
              <div className={cn("grid place-items-center size-6 rounded-full text-[11px] font-semibold transition-all shrink-0", done
              ...
              <span className={cn("text-xs font-medium transition-colors hidden sm:inline", active ? "text-zinc-900" : done ? "text-zinc-500" : "text-zinc-500")}>
                {it.label}
              </span>
            </div>
            {idx < items.length - 1 && (<div className={cn("mx-3 h-px flex-1 transition-colors", done ? "bg-emerald-500/50" : "bg-zinc-200")}/>)}
          </div>);
        })}
    </div>);
}
/* -------------------- Passo 1 · Acesso -------------------- */
function StepCredentials({ form, update, captchaRef, onNext, }) {
    ...
    const canNext = emailOk && strength.valid && matches && (!recaptchaEnabled || !!form.captchaToken);
    ...
    return (<form className="space-y-5" onSubmit={(e) => {
      ...
      <Field label="E-mail" icon={<Mail className="size-3.5"/>} error={emailError}>
        <NeoInput type="email" autoComplete="email" value={form.email} onChange={(e) => update("email", e.target.value)} onBlur={() => setTouched((t) => ({ ...t, email: true }))} placeholder="voce@empresa.com.br" aria-invalid={!!emailError} required/>
      </Field>

      <Field label="Senha" icon={<Lock className="size-3.5"/>} error={pwError}>
        <div className="relative">
          <NeoInput type={showPw ? "text" : "password"} value={form.password} onChange={(e) => update("password", e.target.value)} onBlur={() => setTouched((t) => ({ ...t, password: true }))} placeholder="Mínimo 8 caracteres" autoComplete="new-password" aria-invalid={!!pwError} className="pr-10" required/>
          <button type="button" onClick={() => setShowPw((v) => !v)} className="absolute right-3 top-1/2 -translate-y-1/2 text-zinc-500 hover:text-zinc-900 transition-colors" tabIndex={-1} aria-label={showPw ? "Ocultar senha" : "Mostrar senha"}>
      ...
      <Field label="Confirmar senha" icon={<Lock className="size-3.5"/>} error={confirmError}>
        <NeoInput type="password" value={form.confirm} onChange={(e) => update("confirm", e.target.value)} onBlur={() => setTouched((t) => ({ ...t, confirm: true }))} placeholder="Repita a senha" autoComplete="new-password" aria-invalid={!!confirmError} required/>
        {form.confirm.length > 0 && matches && (<p className="text-xs mt-1.5 text-emerald-600 flex items-center gap-1">
            <CheckCircle2 className="size-3"/> As senhas coincidem
          </p>)}
      </Field>

      {recaptchaEnabled && (<div className="flex justify-center pt-1">
          <div className="w-[237px] h-[61px] sm:w-[304px] sm:h-[78px] max-w-full rounded-md overflow-hidden ring-1 ring-zinc-200">
            <div className="origin-top-left scale-[0.78] sm:scale-100 w-[304px] h-[78px]">
              {recaptchaSiteKey ? (<ReCAPTCHA ref={captchaRef} sitekey={recaptchaSiteKey} theme="light" onChange={(token) => update("captchaToken", token)} onExpired={() => update("captchaToken", null)} onErrored={() => update("captchaToken", null)}/>) : (<div className="w-[304px] h-[78px] bg-white animate-pulse rounded-md"/>)}
            </div>
          </div>
        </div>)}
...
function StrengthMeter({ strength }) {
    ...
    return (<div className="mt-2.5 space-y-2">
      <div className="grid grid-cols-4 gap-1">
        {[1, 2, 3, 4].map((i) => (<div key={i} className={cn("h-1 rounded-full transition-all", strength.score >= i ? colors[strength.score] : "bg-zinc-200")}/>))}
      </div>
      <div className="flex items-center justify-between gap-2">
        ...
        {strength.score > 0 && (<span className={cn("text-[10px] font-medium shrink-0", strength.valid ? "text-emerald-600" : "text-zinc-500")}>
...
function Check({ ok, children }) {
    return (<span className={cn("inline-flex items-center gap-1 transition-colors", ok ? "text-emerald-600" : "text-zinc-500")}>
      <span className={cn("size-1.5 rounded-full", ok ? "bg-emerald-400" : "bg-zinc-300")}/>
      {children}
    </span>);
}
/* -------------------- Passo 2 · Identidade -------------------- */
function StepIdentity({ role, form, update, onBack, onNext, }) {
    const [touched, setTouched] = useState({});
    ...
    return (<form className="space-y-4" onSubmit={(e) => {
      ...
      {isImob && (<Field label="Razão Social" icon={<Building2 className="size-3.5"/>} error={touched.companyName && !companyValid ? "Informe a razão social" : undefined}>
          <NeoInput value={form.companyName} onChange={(e) => update("companyName", e.target.value)} onBlur={() => touch("companyName")} placeholder="Sua Imobiliária LTDA" required/>
        </Field>)}

      <Field label={isImob ? "Nome do responsável" : "Nome completo"} icon={<User className="size-3.5"/>} error={touched.fullName && !fullNameValid ? "Informe um nome válido (mín. 3 letras)" : undefined}>
        <NeoInput value={form.fullName} onChange={(e) => update("fullName", e.target.value)} onBlur={() => touch("fullName")} placeholder={isImob ? "Responsável pela conta" : "Como aparece no documento"} autoComplete="name" required/>
      </Field>

      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <Field label={isImob ? "CNPJ" : "CPF"} icon={<IdCard className="size-3.5"/>} error={touched.document && form.document.length > 0 && !docValid ? `${isImob ? "CNPJ" : "CPF"} inválido` : undefined}>
          <NeoInput value={form.document} onChange={(e) => update("document", isImob ? maskCNPJ(e.target.value) : maskCPF(e.target.value))} onBlur={() => touch("document")} placeholder={isImob ? "00.000.000/0000-00" : "000.000.000-00"} inputMode="numeric" required/>
        </Field>

        <Field label={isImob ? "Telefone comercial" : "Telefone"} icon={<Phone className="size-3.5"/>} error={touched.phone && form.phone.length > 0 && !phoneValid ? "Telefone inválido" : undefined}>
          <NeoInput value={form.phone} onChange={(e) => update("phone", maskPhone(e.target.value))} onBlur={() => touch("phone")} placeholder="(11) 99999-9999" inputMode="tel" autoComplete="tel" required/>
        </Field>
      </div>

      {!isImob && (<Field label="Data de nascimento" icon={<Calendar className="size-3.5"/>} error={touched.birthDate && form.birthDate && !birthValid ? "Data inválida" : undefined}>
          <NeoInput type="date" value={form.birthDate} onChange={(e) => update("birthDate", e.target.value)} onBlur={() => touch("birthDate")} max={new Date().toISOString().slice(0, 10)} required/>
        </Field>)}
...
/* -------------------- Passo 3 · Termos -------------------- */
function StepTerms({ form, update, submitting, onBack, onSubmit, }) {
    ...
    return (<div className="space-y-5">
      ...
      <div className="space-y-3">
        <label className="flex items-start gap-3 cursor-pointer group">
          <Checkbox checked={form.acceptTerms} onCheckedChange={(c) => update("acceptTerms", c === true)} className="mt-0.5 border-zinc-700 data-[state=checked]:bg-violet-600 data-[state=checked]:border-violet-600"/>
        ...
        <label className="flex items-start gap-3 cursor-pointer group">
          <Checkbox checked={form.acceptLgpd} onCheckedChange={(c) => update("acceptLgpd", c === true)} className="mt-0.5 border-zinc-700 data-[state=checked]:bg-violet-600 data-[state=checked]:border-violet-600"/>
          <span className="text-xs text-zinc-700 leading-relaxed group-hover:text-zinc-900 transition-colors">
      ...
      <div className="flex gap-3 pt-1">
        ...
        <Button type="button" onClick={onSubmit} disabled={!canSubmit} className="flex-1 h-11 bg-violet-600 hover:bg-violet-500 text-white font-medium disabled:opacity-40">
          {submitting ? (<>
...
function Field({ label, icon, error, children, }) {
    return (<div className="space-y-1.5">
      ...
      {children}
      {error && (<p className="text-xs text-red-400 flex items-center gap-1" role="alert">
          <AlertCircle className="size-3"/> {error}
        </p>)}
    </div>);
}
const NeoInput = (props) => (<Input {...props} className={cn("h-11 bg-white border-zinc-200 text-zinc-900 placeholder:text-zinc-400", "focus-visible:ring-2 focus-visible:ring-violet-500/50 focus-visible:border-violet-500 transition-colors", "aria-[invalid=true]:border-red-500/70 aria-[invalid=true]:focus-visible:ring-red-500/40", props.className)}/>);
function NextButton({ disabled, children }) {
    return (<Button type="submit" disabled={disabled} className="w-full h-11 bg-violet-600 hover:bg-violet-500 text-white font-medium disabled:opacity-40 transition-all">
      {children}
    </Button>);
}
function BackButton({ onClick, disabled }) {
    return (<Button type="button" variant="outline" onClick={onClick} disabled={disabled} className="h-11 bg-transparent border-zinc-200 text-zinc-700 hover:bg-zinc-50 hover:text-zinc-900">
      <ArrowLeft className="size-4 mr-1"/> Voltar
    </Button>);
}
/* -------------------- Sucesso -------------------- */
function SuccessPanel({ role, email }) {
    return (<div className="rounded-2xl border border-zinc-200 bg-white shadow-sm p-6 sm:p-8">
      ...
      <div className="flex justify-center">
        <div className="rounded-xl bg-white p-3 ring-1 ring-zinc-200">
          <img src={appQrCode.url} alt="QR code para baixar o app Nexo" className="size-56 sm:size-64 rounded-md object-contain"/>
        </div>
      </div>
      ...
      <div className="mt-6 flex flex-col sm:flex-row gap-2.5">
        ...
        <a href={appQrCode.url} download="nexo-app-qrcode.png" className="flex-1">
          <Button type="button" variant="outline" className="w-full h-11 bg-transparent border-zinc-200 text-zinc-700 hover:bg-zinc-50 hover:text-zinc-900">
