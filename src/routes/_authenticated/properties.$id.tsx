...
export const Route = createFileRoute("/_authenticated/properties/$id")({
  head: ({ params }) => ({
    meta: [{ title: `Imóvel ${params.id.slice(0, 8)} — Nexo` }],
  }),
...
function PropertyDetailPage() {
  ...
  const photosQ = usePropertyPhotos(id);
  ...
  const derived = useMemo(() => {
    ...
    const propDocs = (documents as any[]).filter((d) => d.property_id === id);
  ...
  }, [property, contracts, installments, maintenances, documents, inspections, tenants, id]);

  if (isLoading) {
    return (
      <div className="p-6 lg:p-8 max-w-7xl mx-auto">
        <Card className="p-12 animate-pulse bg-muted/10 h-64" />
      </div>
    );
  }
  ...
  if (!property) {
    return (
      <div className="p-6 lg:p-8 max-w-7xl mx-auto space-y-4">
        <Button variant="ghost" onClick={() => navigate({ to: "/properties" })}>
  ...
  return (
    <div className="p-6 lg:p-8 max-w-7xl mx-auto space-y-6">
      <div className="flex items-center justify-between gap-3 flex-wrap">
        ...
        >
          <ArrowLeft className="size-4 mr-2" /> Imóveis
        </Button>
        <Dialog>
          <DialogTrigger asChild>
            <Button variant="outline" size="sm">
              <Pencil className="size-4 mr-2" /> Editar imóvel
            </Button>
          </DialogTrigger>
          <PropertyFormDialog editing={property} onDone={() => {}} />
        </Dialog>
      </div>
          ...
          <div className="p-5 lg:p-6 flex flex-col gap-4 min-w-0">
            ...
            <div className="flex flex-wrap gap-4 text-sm text-muted-foreground pt-2 border-t border-border/50">
              <span className="inline-flex items-center gap-1.5">
                <Bed className="size-3.5" /> {property.bedrooms} quartos
              </span>
              <span className="inline-flex items-center gap-1.5">
                <Bath className="size-3.5" /> {property.bathrooms} banheiros
              </span>
              <span className="inline-flex items-center gap-1.5">
                <Car className="size-3.5" /> {property.garages} vagas
              </span>
              {property.area_total ? (
                <span className="inline-flex items-center gap-1.5">
                  <Ruler className="size-3.5" /> {property.area_total} m²
                </span>
              ) : null}
            </div>
          </div>
      ...
      <Tabs defaultValue="resumo">
        ...
        <TabsContent value="resumo" className="space-y-4 mt-4">
          <div className="grid md:grid-cols-2 gap-4">
            <Card className="p-5 space-y-2">
              <h3 className="font-semibold">Dados do imóvel</h3>
              ...
              <InfoRow label="Transação" value={property.tipo_transacao} capitalize />
            </Card>
            <Card className="p-5 space-y-2">
              <h3 className="font-semibold">Valores</h3>
              <InfoRow label="Aluguel" value={formatBRL(Number(property.rent_price))} />
              <InfoRow label="Condomínio" value={formatBRL(Number(property.condo_fee))} />
              ...
              />
              <InfoRow
                label="Comissão do proprietário"
        ...
        <TabsContent value="financeiro" className="space-y-4 mt-4">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            ...
            <StatCard label="Receita YTD" value={formatBRL(d.ytdRevenue)} tone="emerald" />
            <StatCard label="A receber" value={formatBRL(d.pending)} tone="amber" />
            <StatCard label="Em atraso" value={formatBRL(d.overdue)} tone="destructive" />
          </div>
          <Card className="p-5">
            <div className="flex items-center justify-between mb-3">
              <h3 className="font-semibold">Últimas parcelas</h3>
              <Button variant="link" size="sm" asChild>
                <Link to="/financials">Ver todas</Link>
              </Button>
            </div>
            <div className="space-y-2 text-sm">
              {d.propInstallments.slice(0, 8).map((i: any) => (
                ...
                >
                  <div className="min-w-0">
                    <div className="truncate">Vencimento {formatDate(i.due_date)}</div>
                  ...
                  <div className="text-right tabular-nums shrink-0">
                    <div className="font-semibold">{formatBRL(Number(i.amount))}</div>
                    {i.paid_amount ? (
        ...
        <TabsContent value="contrato" className="space-y-4 mt-4">
          {d.activeContract ? (
            <Card className="p-5 space-y-2">
              ...
              <InfoRow label="Início" value={formatDate(d.activeContract.start_date)} />
              ...
              />
              <InfoRow label="Dia de vencimento" value={String(d.activeContract.due_day)} />
              <InfoRow
            ...
            <Card className="p-8 text-center">
              <CalendarClock className="size-10 mx-auto text-muted-foreground mb-2" />
              <p className="text-muted-foreground">Nenhum contrato ativo.</p>
              <Button asChild size="sm" className="mt-3">
                <Link to="/contracts">Criar contrato</Link>
              </Button>
            </Card>
          )}
          {d.propContracts.length > 1 ? (
            <Card className="p-5">
              <h3 className="font-semibold mb-3">Histórico de contratos</h3>
              <div className="space-y-2 text-sm">
                {d.propContracts
          ...
          {d.activeTenant ? (
            <Card className="p-5 space-y-2">
              <div className="flex items-center gap-2 mb-2">
                ...
                <h3 className="font-semibold">{d.activeTenant.full_name}</h3>
              </div>
              <InfoRow label="CPF/CNPJ" value={(d.activeTenant as any).document ?? "—"} />
              <InfoRow
                label="E-mail"
                value={d.activeTenant.email}
                icon={Mail}
              />
              <InfoRow
                label="Telefone"
                value={d.activeTenant.phone}
                icon={Phone}
              />
            </Card>
          ) : (
            <Card className="p-8 text-center">
              <User2 className="size-10 mx-auto text-muted-foreground mb-2" />
              <p className="text-muted-foreground">Sem inquilino ativo.</p>
            </Card>
          )}
        </TabsContent>
        ...
        <TabsContent value="documentos" className="space-y-3 mt-4">
          {d.propDocs.length === 0 ? (
            <Card className="p-8 text-center">
              ...
              <p className="text-muted-foreground">Nenhum documento neste imóvel.</p>
              <Button asChild size="sm" className="mt-3">
        ...
        <TabsContent value="vistorias" className="space-y-3 mt-4">
          {d.propInsp.length === 0 ? (
            <Card className="p-8 text-center">
              <ClipboardCheck className="size-10 mx-auto text-muted-foreground mb-2" />
              <p className="text-muted-foreground">Nenhuma vistoria registrada.</p>
              <Button asChild size="sm" className="mt-3">
                <Link to="/vistorias">Registrar vistoria</Link>
              </Button>
            </Card>
          ) : (
            <Card className="p-5">
                ...
                {d.propInsp.map((insp: any) => (
                  ...
                  >
                    <div>
                      ...
                      <div className="text-xs text-muted-foreground">
                        {formatDate(insp.inspection_date)} · {insp.general_condition}
                      </div>
                    </div>
                    <Badge variant="outline" className="capitalize">
                      {insp.status}
        ...
        <TabsContent value="manutencoes" className="space-y-4 mt-4">
          <div className="grid grid-cols-3 gap-3">
            <StatCard label="Abertas" value={String(d.openMaint.length)} tone="amber" />
            <StatCard label="Concluídas" value={String(d.doneMaint.length)} tone="emerald" />
            <StatCard label="Gasto total" value={formatBRL(d.maintSpend)} />
          </div>
          {d.propMaint.length === 0 ? (
            <Card className="p-8 text-center">
              <Wrench className="size-10 mx-auto text-muted-foreground mb-2" />
              <p className="text-muted-foreground">Sem manutenções.</p>
            </Card>
          ) : (
            <Card className="p-5">
              <div className="space-y-2 text-sm">
                {d.propMaint.slice(0, 20).map((m: any) => (
                  <div
                    key={m.id}
                    className="flex items-center justify-between border-b border-border/40 pb-2 last:border-0"
                  >
                    ...
                    <div className="tabular-nums shrink-0">
                      {formatBRL(
                        Number(m.payment_paid_amount || m.cost || m.budget_amount || 0),
                      )}
                    </div>
                  </div>
                ))}
...
function MiniStat({
  ...
  return (
    <div className="rounded-lg border border-border/60 bg-muted/20 px-3 py-2 min-w-0">
      <div className="flex items-center gap-1.5 text-[11px] uppercase text-muted-foreground tracking-wide truncate">
        <Icon className="size-3" /> {label}
      </div>
      <div className={`font-bold tabular-nums truncate ${toneClass}`}>{value}</div>
    </div>
  );
}
    ...
    <div className="space-y-3">
      {items.map((it, idx) => (
        <div key={idx} className="flex gap-3 text-sm">
          <div className="text-xs text-muted-foreground w-24 shrink-0 tabular-nums pt-0.5">
            {formatDate(it.date)}
          </div>
